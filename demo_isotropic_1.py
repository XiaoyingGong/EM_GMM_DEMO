...
# 最大化 对各个值求偏导
def Maximization(data, response, N, K):
    dim = data.shape[1]
    # 初始化
    mus = np.zeros([K, dim])
    sigmas = np.zeros(K)
...
def EM_algorithm(data, initial_mu, initial_sigma, initial_alpha, max_iterative, is_draw=True):
    ...
    for i in range(max_iterative):
        ...
        # M-step
        mus, sigmas, alphas = Maximization(data, response, N, K)
        if is_draw:
...
def process_1():
    ...
    # 计算与更新
    initial_mus = np.array([[-1, -0.9], [-1, -1], [4, -1]], dtype=np.float)
    ...
    mus_results, sigmas_results, weights_results =\
        EM_algorithm(data, initial_mus, initial_sigmas, initial_alpha, 20)
    end_time = time.time()
